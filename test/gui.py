...
class MainWindow(wx.Dialog):
    ...
    def initial_ui(self):
        ...
        text2 = wx.StaticText(self, label="未設置")
        ...
        text4 = wx.StaticText(self, label="未設置")
        ...
        text6 = wx.StaticText(self, label="未設置")
        ...
        text8 = wx.StaticText(self, label="未設置")
        ...
        text10 = wx.StaticText(self, label="未設置")
    ...
    def on_set_start_time(self, event):
        SettingWindow(None, title="幾秒後開始運行程式", min_range=5, max_range=600, unit="秒")

    def on_set_duration(self, event):
        SettingWindow(None, title="每隔幾秒後播放下一張投影片", min_range=5, max_range=600, unit="秒")

    def on_set_expiration_date(self, event):
        SettingWindow(None, title="刪除超過幾天的檔案", min_range=1, max_range=180, unit="天")

    def on_set_amount_of_keep_file(self, event):
        SettingWindow(None, title="保留幾個最近建立的檔案", min_range=0, max_range=100, unit="個")
...
class SettingWindow(wx.Dialog):
    ...
    def initial_ui(self, min_range, max_range, unit):
        ...
        text = "設定數值： (" + str(min_range) + "~" + str(max_range) + " " + unit + ")"
        ...
        sizer1.Add(textctrl1, pos=(0, 1), span=(1, 2), flag=wx.ALL | wx.EXPAND | wx.ALIGN_CENTER_VERTICAL, border=5)
